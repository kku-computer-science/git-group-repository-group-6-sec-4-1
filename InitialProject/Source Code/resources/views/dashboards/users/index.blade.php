                                            ...
                                            <div class="col-12 mb-3 last-card-margin">
                                                <div class="border border-3" style="border-color: #495057; border-radius: 0.375rem; padding: 15px; background-color: #f8f9fa; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                                                    ...
                                                    <div class="d-flex justify-content-between">
                                                        ...
                                                        <a href="{{ url('/logs/http?http_search=' . ($notification['status_code'] ?? '') . '&start_date=' . request('selected_date') . '&end_date=' . request('selected_date') . '&ip=' . urlencode($notification['ip'] ?? '') . '&url=' . urlencode($notification['url'] ?? '')) }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Dismiss Button Logic with Alert
        const dismissButtons = document.querySelectorAll('.dismiss-btn');
        if (dismissButtons.length === 0) {
            console.warn('No dismiss buttons found in the DOM');
        }
        ...
        dismissButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (!id) {
                    console.error('No data-id found on dismiss button');
                    alert('Error: Notification ID not found');
                    return;
                }

                const notificationContainer = this.closest('.col-12'); // หากล่องการแจ้งเตือนทั้งหมด
                ...
                .then(data => {
                    if (data.success) {
                        if (notificationContainer) {
                            notificationContainer.remove(); // ลบกล่องการแจ้งเตือนทั้งหมด
                            console.log(`Notification ${id} dismissed successfully`);
                            alert('Notification dismissed successfully'); // เพิ่ม Alert เมื่อลบสำเร็จ
                        } else {
                            console.warn(`Could not find notification container for ID ${id}`);
                            alert('Notification dismissed but UI update failed');
                        }
                    } else {
